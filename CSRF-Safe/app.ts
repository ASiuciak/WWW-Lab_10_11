...
app.get('/', csrfProtection, function(req, res) {
    res.render('index', { title: 'Meme market', message: 'Hello there!', memes:  getMostExpensive(), csrfToken: req.csrfToken()})
});

app.get('/meme/:memeId', csrfProtection, function (req, res) {
       let meme = getMeme(parseInt(req.params.memeId));
       if (meme === null) {
           res.render('error', { text: 'No meme with such id', csrfToken: req.csrfToken()})
       } else {
           let list = meme.getHistory()
           let token = req.csrfToken();
           console.log(token);
           res.render('history', { title: 'History of meme ' + meme.id, meme: meme, prices: list, csrfToken: req.csrfToken()})
       }
})
